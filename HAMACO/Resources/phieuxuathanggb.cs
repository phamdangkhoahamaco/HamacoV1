...
namespace HAMACO.Resources
{
    class phieuxuathanggb
    {
        ...
        public void tsbtpck(string a, Form1 F, DevExpress.XtraGrid.Views.Grid.GridView view,string roleid,string subsys, string ngaychungtu, string userid, string branchid,string tsbt,DataTable khach,DataTable hang)
        {
            ...
            u.myac = new Frm_chuyenkhonb.ac(F.refreshxhgb);
            u.getactive(a);
            u.getsub(subsys);
            u.getkhach(khach);
            u.gethang(hang);
            ...
            ToolStripButton tsbtsua, ToolStripButton tsbtxoa, ToolStripButton tsbtcat, ToolStripButton tsbtboghi, ToolStripButton tsbtghiso, ToolStripButton tsbtnap, ToolStripButton tsbtin, string ngaychungtu, string userid, string branchid, TextEdit txtngh, TextEdit txtptvc, DevExpress.XtraEditors.Repository.RepositoryItemTextEdit dongia, DevExpress.XtraEditors.Repository.RepositoryItemTextEdit thanhtien, TextEdit txtpnv, TextEdit txtcth, DevExpress.XtraEditors.Repository.RepositoryItemTextEdit chiphi, TextEdit txtnhd, TextEdit txtms, TextEdit txtkhhd, TextEdit txtshd, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string tsbt, DataTable khach, DataTable hang, CheckEdit xuat, CheckEdit nhan)
        {
            ...
            {
                DataTable da = new DataTable();
                da = gen.GetTable("select  InventoryItemCode,Quantity,QuantityConvert,Description,InventoryItemName,a.UnitPrice,a.Amount,Cost,a.UnitPriceOC,a.AmountOC from INTransferBranchDetail a,InventoryItem b where a.InventoryItemID=b.InventoryItemID and RefID='" + role + "' order by SortOrder");
                for (int i = 0; i < da.Rows.Count; i++)
                {
                    DataRow dr = dt.NewRow();
                    dr[0] = da.Rows[i][0].ToString();
                ...
                F.Text = "Xem phiếu xuất hàng gửi bán";

                da = gen.GetTable("select AccountingObjectCode,a.Contactname,JournalMemo,InvSeries,RefDate,PostedDate,RefNo,OutwardStockID,Posted,InvNo,Cancel,ShippingNo,RefNoIn,InwardStockID,No,InvDate,IsExport,PostVersion  from INTransferBranch a, AccountingObject b where a.AccountingObjectID=b.AccountingObjectID  and RefID='" + role + "'");
            ...
            {
                F.Text = "Thêm phiếu xuất hàng gửi bán";
                if (role == null)
                    ledv.ItemIndex = 0;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (active == "0")
                    {
                        ...
                        try
                        {
                            string ton = gen.GetString("select * from INTransferBranch where RefNoIn='" + txtpnv.Text + "'");
                        ...
                        gen.ExcuteNonquery("insert into INTransferBranch(RefID,RefType,RefDate,PostedDate,RefNo,RefNoIn,AccountingObjectID,AccountingObjectName,Contactname,JournalMemo,Posted,OutwardStockID,InwardStockID,ShippingNo,TotalAmount,CostAmount,No,InvSeries,InvNo,InvDate,UserID,IsExport) values(newid(),101,'" + denct.EditValue.ToString() + "','" + denht.EditValue.ToString() + "','" + txtsct.Text + "','" + txtpnv.Text + "','" + dt + "',N'" + txtname.Text + "',N'" + txtngh.Text + "',N'" + txtldn.Text + "','False','" + dv + "','" + dvn + "',N'" + txtptvc.Text + "','" + tongtien + "','" + chiphi + "','" + txtms.Text + "','" + txtkhhd.Text + "','" + txtshd.Text + "','" + txtnhd.EditValue.ToString() + "','" + userid + "','"+xuat.EditValue.ToString()+"')");
                        string refid = gen.GetString("select * from INTransferBranch where RefNo='" + txtsct.Text + "'");
                        F.getrole(refid);
                        gen.ExcuteNonquery("insert into HACHTOAN(RefDeteail,RefID,RefNo,DebitAccount,CreditAccount,Amount,AccountingObjectID,StockID,JournalMemo,RefDate,AccountingObjectIDMain) values(newid(),'" + refid + "','" + txtsct.Text + "','157','156','" + tongcong.ToString() + "','" + dt + "','" + dv + "',N'" + txtldn.Text + "','" + denct.EditValue.ToString() + "','"+dt+"')");
                        ...
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into INTransferBranchDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,UnitPrice,Amount,Cost,UnitPriceOC,AmountOC) values(newid(),'" + refid + "','" + detail[i, 1] + "','" + detail[i, 2] + "'," + i + ",'" + detail[i, 0] + "',N'" + detail[i, 3] + "','" + detail[i, 4] + "','" + detail[i, 5] + "','" + detail[i, 6] + "','" + detail[i, 7] + "','" + detail[i, 8] + "')");
                    ...
                    else
                    {
                        ...
                        DataTable hangchuyen = gen.GetTable("select InventoryItemID,Quantity,QuantityConvert from INTransferBranchDetail where RefID='" + role + "' ");
                        ...
                        gen.ExcuteNonquery("insert into HACHTOAN(RefDeteail,RefID,RefNo,DebitAccount,CreditAccount,Amount,AccountingObjectID,StockID,JournalMemo,RefDate,AccountingObjectIDMain) values(newid(),'" + role + "','" + txtsct.Text + "','157','156','" + tongcong.ToString() + "','" + dt + "','" + dv + "',N'" + txtldn.Text + "','" + denct.EditValue.ToString() + "','"+dt+"')");
                        ...
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into INTransferBranchDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,UnitPrice,Amount,Cost,UnitPriceOC,AmountOC) values(newid(),'" + role + "','" + detail[i, 1] + "','" + detail[i, 2] + "'," + i + ",'" + detail[i, 0] + "',N'" + detail[i, 3] + "','" + detail[i, 4] + "','" + detail[i, 5] + "','" + detail[i, 6] + "','" + detail[i, 7] + "','" + detail[i, 8] + "')");
                            for (int j = 0; j < hangton.Rows.Count; j++)
                            {
                                if (gridView1.GetRowCellValue(i, "Mã hàng").ToString() == hangton.Rows[j][3].ToString())
                                {
        ...
        /*public void themsct(string ngaychungtu, TextEdit txtsct, string ledv, string ledvn, TextEdit txtpnv, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau)
        {
            ...
            string sophieuvo = dvn + "-" + ledvn + "-NHGB";
                try
                {
        ...
        public void checktruoc(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_chuyenkhonb F, string ngay, string mk,string pt)
        {
            try
            {
                ...
                if (vt == 0)
                    if(pt=="tsbtpnhgb")
                        id = gen.GetString("select Top 1 * from INTransferBranch where RefNoIn < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and InwardStockID='" + idkho + "' order by RefNoIn DESC");
                    else
                        id = gen.GetString("select Top 1 * from INTransferBranch where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and OutwardStockID='" + idkho + "' order by RefNo DESC");
                else
                {
                    if(pt=="tsbtpnhgb")
                        id = gen.GetString("select Top 1 * from INTransferBranch where RefNoIn < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and InwardStockID='" + idkho + "' order by RefNoIn ASC");
                    else
                        id = gen.GetString("select Top 1 * from INTransferBranch where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and OutwardStockID='" + idkho + "' order by RefNo ASC");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INTransferBranch where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and OutwardStockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
        ...
        public void checktruocsaunhap(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INTransferBranch where RefNoIn > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and InwardStockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
