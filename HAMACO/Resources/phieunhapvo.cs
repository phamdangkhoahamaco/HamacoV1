    ...
    class phieunhapvo
    {
        ...
        public void tsbtdeletepnk(DevExpress.XtraGrid.Views.Grid.GridView view, Form1 F)
        {
            try
            {
                ...
                if (gen.GetString("select Posted from INInwardSU where RefID='" + name + "'") == "True")
                {
                ...
                {
                    gen.ExcuteNonquery("delete from INInwardSU where RefID='" + name + "'");
                    gen.ExcuteNonquery("delete from INInwardSUDetail where RefID='" + name + "'");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INInwardSU where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and StockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
