    ...
    class phieuxuathanggbvlpg
    {
        ...
        public void loadpck(DevExpress.XtraGrid.GridControl lvpq, DevExpress.XtraGrid.Views.Grid.GridView view, string sql)
        {
            ...
            for (int i = 0; i < temp.Rows.Count; i++)
            {
                ...
                dr[4] = gen.GetString("select StockCode from Stock where StockID='" + temp.Rows[i][12].ToString() + "'");
                dr[5] = gen.GetString("select StockName from Stock where StockID='" + temp.Rows[i][12].ToString() + "'");
                dr[6] = temp.Rows[i][33].ToString();
                if (temp.Rows[i][34].ToString() == "True")
                    dr[7] = "True";
                else
                    dr[7] = "False";

                dr[8] = gen.GetString("select StockCode from Stock where StockID='" + temp.Rows[i][13].ToString() + "'");
                dt.Rows.Add(dr);
            }
            lvpq.DataSource = dt;
        ...
        public void tsbtdeletepnk(DevExpress.XtraGrid.Views.Grid.GridView view, Form1 F)
        {
            try
            {
                ...
                if (gen.GetString("select Posted from INTransferBranchSU where RefID='" + name + "'") == "True")
                {
                    XtraMessageBox.Show("Phiếu đã được ghi sổ không thể xóa.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (XtraMessageBox.Show("Bạn có chắc muốn xóa phiếu xuất hàng gửi bán vỏ LPG " + view.GetRowCellValue(view.FocusedRowHandle, "Số chứng từ").ToString() + "?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    gen.ExcuteNonquery("delete from INTransferBranchSU where RefID='" + name + "'");
                    gen.ExcuteNonquery("delete from INTransferBranchSUDetail where RefID='" + name + "'");
            ...
            catch { XtraMessageBox.Show("Vui lòng chọn phiếu xuất hàng gửi bán vỏ LPG trước khi xóa.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information); }
        }
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INTransferBranchSU where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and OutwardStockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
        ...
        public void checktruocsaunhap(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INTransferBranchSU where RefNoIn > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and InwardStockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
