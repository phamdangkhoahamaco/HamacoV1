...
namespace HAMACO.Resources
{
    class phieuthunh
    {
        ...
        public void loadptnh(DevExpress.XtraGrid.GridControl lvpq, DevExpress.XtraGrid.Views.Grid.GridView view, string sql)
        {
        ...
        public void tsbtptnh(string a, Form1 F, DevExpress.XtraGrid.Views.Grid.GridView view,string roleid,string subsys,string ngaychungtu,string userid,DataTable khach)
        {
            try
            {
                Frm_phieuthu u = new Frm_phieuthu();
                u.myac = new Frm_phieuthu.ac(F.refreshptnh);
                u.getactive(a);
            ...
            catch { MessageBox.Show("Vui lòng chọn phiếu thu ngân hàng trước khi sửa."); }
        }


        public void loadnh(CheckEdit cechd, string active, string role, DevExpress.XtraGrid.GridControl gridControl1, GridView gridView1, TextEdit txtsct, ComboBoxEdit cbldt, LookUpEdit ledv, DateEdit denct, DateEdit denht,
    ...
    ToolStripButton tsbtsua, ToolStripButton tsbtxoa, ToolStripButton tsbtcat, ToolStripButton tsbtboghi, ToolStripButton tsbtghiso, ToolStripButton tsbtnap, ToolStripButton tsbtin, string ngaychungtu, string tsbt, ComboBoxEdit cbthue, TextEdit txthtt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, DataTable khach, string userid, DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit rpmanganh, DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit rpmachiphi,SearchLookUpEdit danhmuc)
        {
            ...
            if (active == "1")
            {
                ...
                da = gen.GetTable("select  a.Description,DebitAccount,CreditAccount,Amount,AccountingObjectCode,InvDate,InvSeries,InvNo,CustomField5,CustomField4 from BADepositDetail a,AccountingObject b where a.AccountingObjectID=b.AccountingObjectID and RefID='" + role + "' order by SortOrder");
            ...
            else
            {
                try
                {
                    F.Text = "Thêm phiếu thu ngân hàng";
                    txthtt.EditValue = 0;
...
//save//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        public void checkptnh(string active, string role, Frm_phieuthu F, GridView gridView1, LookUpEdit ledt, LookUpEdit ledv, ComboBoxEdit cbldt, TextEdit txtsct, TextEdit txtname, TextEdit txtdc,
            ...
            ToolStripButton tsbtcat, ToolStripButton tsbtin, ToolStripButton tsbtsua, ToolStripButton tsbtnap, string ngaychungtu, ComboBoxEdit cbthue, string userid, TextEdit txthtt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau,SearchLookUpEdit danhmuc)
        {
            try
            {
                ...
                for (int i = 0; i < gridView1.RowCount - 1; i++)
                {
                    ...
                    detail[i, 3] = gridView1.GetRowCellValue(i, "Số hóa đơn").ToString();
                    detail[i, 4] = gridView1.GetRowCellValue(i, "Loại hóa đơn").ToString();
                ...
                else
                {
                    ...
                    if (active == "0")
                    {
                        try
                        {
                            string ton = gen.GetString("select * from BADeposit where RefNo='" + txtsct.Text + "'");
                            themsct(ngaychungtu, txtsct,tsbttruoc,tsbtsau,userid);
                        ...
                        gen.ExcuteNonquery("insert into BADeposit(RefID,RefType,RefDate,PostedDate,RefNo,AccountingObjectID,AccountingObjectName,AccountingObjectAddress,Contactname,JournalMemo,DocumentIncluded,Posted,StockID,AccountingObjectType,TotalAmount,Tax,UserID,ExDate,RefOrder) values(newid(),101,'" + denct.EditValue.ToString() + "','" + denht.EditValue.ToString() + "','" + txtsct.Text + "','" + dt + "',N'" + txtname.Text + "','" + txtdc.Text + "',N'" + txtnn.Text + "',N'" + txtldn.Text + "',N'" + txtctg.Text + "','False','" + dv + "','" + ldt + "','" + tong + "','" + cbthue.Text + "','" + userid + "','" + txthtt.EditValue.ToString() + "','" + danhmuc.EditValue.ToString() + "')");
                        string refid = gen.GetString("select * from BADeposit where RefNo='" + txtsct.Text + "'");
                        F.getrole(refid);
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            /*if (detail[i, 2] == "")
                            {
                                gen.ExcuteNonquery("insert into BADepositDetail(RefDetailID,RefID,Description,DebitAccount,CreditAccount,Amount,AccountingObjectID,SortOrder,CustomField5) values(newid(),'" + refid + "',N'" + detail[i, 7] + "','" + detail[i, 1] + "','" + detail[i, 0] + "','" + detail[i, 5] + "','" + detail[i, 6] + "'," + i + ",'" + detail[i, 8] + "')");
                            }
                            else
                            {*/
                            gen.ExcuteNonquery("insert into BADepositDetail(RefDetailID,RefID,Description,DebitAccount,CreditAccount,Amount,AccountingObjectID,InvDate,InvSeries,InvNo,SortOrder,CustomField5,CustomField4) values(newid(),'" + refid + "',N'" + detail[i, 7] + "','" + detail[i, 1] + "','" + detail[i, 0] + "','" + detail[i, 5] + "','" + detail[i, 6] + "'," + detail[i, 2] + ",'" + detail[i, 3] + "','" + detail[i, 4] + "'," + i + ",'" + detail[i, 8] + "',N'" + detail[i, 9] + "')");
                            //}
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            /*if (detail[i, 2] == "")
                            {
                                gen.ExcuteNonquery("insert into BADepositDetail(RefDetailID,RefID,Description,DebitAccount,CreditAccount,Amount,AccountingObjectID,SortOrder,CustomField5) values(newid(),'" + role + "',N'" + detail[i, 7] + "','" + detail[i, 1] + "','" + detail[i, 0] + "','" + detail[i, 5] + "','" + detail[i, 6] + "'," + i + ",'" + detail[i, 8] + "')");
                            }
                            else
                            {*/
        ...
        public void tsbtdeleteptnh(DevExpress.XtraGrid.Views.Grid.GridView view, Form1 F)
        {
            try
            {
                string name = view.GetRowCellValue(view.FocusedRowHandle, "ID").ToString();
                if (gen.GetString("select Posted from BADeposit where RefID='" + name + "'") == "True")
                {
                    XtraMessageBox.Show("Phiếu đã được ghi sổ không thể xóa.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (XtraMessageBox.Show("Bạn có chắc muốn xóa phiếu thu ngân hàng " + view.GetRowCellValue(view.FocusedRowHandle, "Số chứng từ").ToString() + "?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    gen.ExcuteNonquery("delete from BADeposit where RefID='" + name + "'");
                    gen.ExcuteNonquery("delete from BADepositDetail where RefID='" + name + "'");
        ...
        public void checktruoc(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_phieuthu F, string ngay,string userid)
        {
            try
            {
                ...
                string id;
                if (vt == 0)
                    id = gen.GetString("select Top 1 * from BADeposit where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and StockID in (select StockID from MSC_UserJoinStock where UserID='" + userid + "') order by RefNo DESC");
                else
                {
                    id = gen.GetString("select Top 1 * from BADeposit where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and StockID in (select StockID from MSC_UserJoinStock where UserID='" + userid + "') order by RefNo ASC");
        ...
        public void themsct(string ngaychungtu, TextEdit txtsct, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau,string userid)
        {
            ...
            string sophieu = donvi + "-" + donvi + "-PTNH";
            
                try
                {
                    string id = gen.GetString("select Top 1 RefNo from BADeposit where Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "'  and SUBSTRING(RefNo,1,2)='" + donvi + "'  order by RefNo DESC");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string sct, string ngaychungtu,string userid)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from BADeposit where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and StockID in (select StockID from MSC_UserJoinStock where UserID='" + userid + "')");
            }
            catch
            {
                tsbtsau.Enabled = false;
