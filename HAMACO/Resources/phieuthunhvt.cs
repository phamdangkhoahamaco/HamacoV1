...
namespace HAMACO.Resources
{
    class phieuthunhvt
    {
        ...
        public void loadptnhvt(DevExpress.XtraGrid.GridControl lvpq, DevExpress.XtraGrid.Views.Grid.GridView view, string sql)
        {
        ...
        public void tsbtptnhvt(string a, Form1 F, DevExpress.XtraGrid.Views.Grid.GridView view,string roleid,string subsys, string ngaychungtu,string userid,DataTable khach,DataTable hang)
        {
            try
            {
            Frm_phieuthuvt u = new Frm_phieuthuvt();
            u.myac = new Frm_phieuthuvt.ac(F.refreshptnhvt);
            u.getactive(a);
            u.getuser(userid);
            u.getroleid(roleid);
            u.gethang(hang);
        ...
        public void loadnhvt(string active, string role, DevExpress.XtraGrid.GridControl gridControl1, GridView gridView1, TextEdit txtsct, ComboBoxEdit cbldt, LookUpEdit ledv, DateEdit denct, DateEdit denht,
            ...
            ToolStripButton tsbtsua, ToolStripButton tsbtxoa, ToolStripButton tsbtcat, ToolStripButton tsbtboghi, ToolStripButton tsbtghiso, ToolStripButton tsbtnap, ToolStripButton tsbtin, string ngaychungtu, string tsbt, ComboBoxEdit cbthue, TextEdit txtshd, TextEdit txtkhhd, TextEdit txtnhd, TextEdit txtcth, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau,DataTable khach,DataTable hang)
        {
            ...
            if (active == "1")
            {
                ...
                da = gen.GetTable("select  DebitAccount,CreditAccount,Amount,InventoryItemCode,a.SalePrice,Quantity from SUBADepositDetail a,InventoryItem b where a.InventoryItemID=b.InventoryItemID and RefID='" + role + "' order by SortOrder");
                ...
                F.Text = "Xem phiếu thu ngân hàng bán vật tư";
                da = gen.GetTable("select AccountingObjectCode,a.Contactname,JournalMemo,DocumentIncluded,RefDate,PostedDate,RefNo,StockCode,Posted,AccountingObjectType,Cancel,Tax,InvDate,InvSeries,InvNo  from SUBADeposit a, AccountingObject b,Stock c where a.AccountingObjectID=b.AccountingObjectID and a.StockID=c.StockID and RefID='" + role + "'");
                try
                {
            ...
            else
            {
                try
                {
                    F.Text = "Thêm phiếu thu ngân hàng bán vật tư";
                    if (role == null)
                        ledv.ItemIndex = 0;
        ...
        //save//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        public void checkptnhvt(string active, string role, Frm_phieuthuvt F, GridView gridView1, LookUpEdit ledt, LookUpEdit ledv, ComboBoxEdit cbldt, TextEdit txtsct, TextEdit txtname, TextEdit txtdc,
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (active == "0")
                    {
                        try
                        {
                            string ton = gen.GetString("select * from SUBADeposit where RefNo='" + txtsct.Text + "'");
                        ...
                        if (nhd == "")
                        {
                            gen.ExcuteNonquery("insert into SUBADeposit(RefID,RefType,RefDate,PostedDate,RefNo,AccountingObjectID,AccountingObjectName,AccountingObjectAddress,Contactname,JournalMemo,DocumentIncluded,Posted,StockID,AccountingObjectType,TotalAmount,Tax,InvSeries,InvNo,EmployeeID) values(newid(),101,'" + denct.EditValue.ToString() + "','" + denht.EditValue.ToString() + "','" + txtsct.Text + "','" + dt + "',N'" + txtname.Text + "','" + txtdc.Text + "',N'" + txtnn.Text + "',N'" + txtldn.Text + "',N'" + txtctg.Text + "','False','" + dv + "','" + ldt + "','" + tong + "','" + cbthue.Text + "','" + txtkhhd.Text + "','" + txtshd.Text + "','"+userid+"')");

                        }
                        else
                        {
                            gen.ExcuteNonquery("insert into SUBADeposit(RefID,RefType,RefDate,PostedDate,RefNo,AccountingObjectID,AccountingObjectName,AccountingObjectAddress,Contactname,JournalMemo,DocumentIncluded,Posted,StockID,AccountingObjectType,TotalAmount,Tax,InvDate,InvSeries,InvNo,EmployeeID) values(newid(),101,'" + denct.EditValue.ToString() + "','" + denht.EditValue.ToString() + "','" + txtsct.Text + "','" + dt + "',N'" + txtname.Text + "','" + txtdc.Text + "',N'" + txtnn.Text + "',N'" + txtldn.Text + "',N'" + txtctg.Text + "','False','" + dv + "','" + ldt + "','" + tong + "','" + cbthue.Text + "','" + txtnhd.EditValue.ToString() + "','" + txtkhhd.Text + "','" + txtshd.Text + "','"+userid+"')");
                        }
                        string refid = gen.GetString("select * from SUBADeposit where RefNo='" + txtsct.Text + "'");
                        F.getrole(refid);
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into SUBADepositDetail(RefDetailID,RefID,DebitAccount,CreditAccount,Amount,Quantity,SortOrder,InventoryItemID,SalePrice) values(newid(),'" + refid + "','" + detail[i, 1] + "','" + detail[i, 0] + "','" + detail[i, 5] + "','" + detail[i, 3] + "'," + i + ",'" + detail[i, 2] + "','" + detail[i, 4] + "')");
        ...
        public void tsbtdeleteptnhvt(DevExpress.XtraGrid.Views.Grid.GridView view, Form1 F)
        {
            try
            {
                string name = view.GetRowCellValue(view.FocusedRowHandle, "ID").ToString();
                if (gen.GetString("select Posted from SUBADeposit where RefID='" + name + "'") == "True")
                {
                    XtraMessageBox.Show("Phiếu đã được ghi sổ không thể xóa.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (XtraMessageBox.Show("Bạn có chắc muốn xóa phiếu thu ngân hàng bán vật tư " + view.GetRowCellValue(view.FocusedRowHandle, "Số chứng từ").ToString() + "?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    gen.ExcuteNonquery("delete from SUBADeposit where RefID='" + name + "'");
                    gen.ExcuteNonquery("delete from SUBADepositDetail where RefID='" + name + "'");
        ...
        public void checktruoc(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_phieuthuvt F, string ngay,string mk)
        {
            try
            {
                ...
                string id;
                if (vt == 0)
                    id = gen.GetString("select Top 1 * from SUBADeposit where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and StockID='" + idkho + "' order by RefNo DESC");
                else
                {
                    id = gen.GetString("select Top 1 * from SUBADeposit where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and StockID='" + idkho + "' order by RefNo ASC");
        ...
        public void themsct(string ngaychungtu, TextEdit txtsct, string mk, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau)
        {
            ...
            string sophieu = "08-" + mk + "-NTVT";
                try
                {
                    string id = gen.GetString("select Top 1 RefNo from SUBADeposit where Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and StockID='" + idkho + "'  order by RefNo DESC");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from SUBADeposit where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and StockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
