    ...
    class phieunhapgas
    {
        ...
        public void loadpnk(DevExpress.XtraGrid.GridControl lvpq, DevExpress.XtraGrid.Views.Grid.GridView view, string sql)
        {
            ...
            for (int i = 0; i < temp.Rows.Count; i++)
            {
                ...
                dr[6] = gen.GetString("select a.refNo from INInwardSU a, INInward b where b.RefID='" + temp.Rows[i][0].ToString() + "' and a.RefID=b.refSUID");
        ...
        public void themsct(string ngaychungtu, TextEdit txtsct, string mk, string branchid, TextEdit txtpnv, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau)
        {
            ...
            string sophieuvo = branch + "-" + mk + "-PNVT";
                ...
                try
                {
                    string id = gen.GetString("select Top 1 RefNo from INInwardSU where Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and StockID='" + idkho + "'  order by RefNo DESC");
                    int ct = Int32.Parse(id.Substring(10, dai)) + 1;
                    for (int i = 0; i < dai - ct.ToString().Length; i++)
                    {
        ...
        public void checktruoc(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_phieunhapgas F, string ngay, string mk)
        {
            try
            {
                ...
                string id;
                if (vt == 0)
                    id = gen.GetString("select Top 1 * from INInward where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and refSUID is not Null and StockID='" + idkho + "' order by RefNo DESC");
                else
                {
                    id = gen.GetString("select Top 1 * from INInward where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and refSUID is not Null and StockID='" + idkho + "' order by RefNo ASC");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INInward where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and refSUID is not Null and StockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
