    ...
    class phieunhapkho
    {
        ...
        public void tsbtpnk(string a, Form1 F, DevExpress.XtraGrid.Views.Grid.GridView view, string roleid, string subsys, string ngaychungtu, string userid, string branchid, DataTable khach, DataTable hang)
        {
            try
            {
                ...
                u.myac = new Frm_phieunhapkho.ac(F.refreshpnk);
                u.getactive(a);
                u.getroleid(roleid);
                u.getsub(subsys);
                u.getpt("pnk");
            ...
            ToolStripButton tsbtboghi, ToolStripButton tsbtghiso, ToolStripButton tsbtnap, ToolStripSplitButton tsbtin, string ngaychungtu, string userid, string branchid, string active, DataTable dt, LookUpEdit lenv, DataTable khach, DataTable hang, DevExpress.XtraEditors.Repository.RepositoryItemTextEdit dongia, DevExpress.XtraEditors.Repository.RepositoryItemTextEdit thanhtien)
        {
            ...
            */
            ...
            /*BandedGridView gridView1 = gridControl1.MainView as BandedGridView;
            gridView1.Bands.Clear();
            // creating the bands layout 
            GridBand bandProdInf = gridView1.Bands.Add("Product Information");
            GridBand bandPriceInf = View.Bands.Add("Price Information");
            GridBand bandOther = View.Bands.Add("Other");
            GridBand bandPrice = bandPriceInf.Children.Add("Price");
            GridBand bandUnits = bandPriceInf.Children.Add("Units");
            // assigning columns to bands 
            colProdId.OwnerBand = bandProdInf;
            colProdName.OwnerBand = bandProdInf;
            colUnitPrice.OwnerBand = bandPrice;
            colQuantity.OwnerBand = bandPrice;
            colDiscount.OwnerBand = bandPrice;
            colInStock.OwnerBand = bandUnits;
            colInOrder.OwnerBand = bandUnits;
            colEAN.OwnerBand = bandOther;*/
        ...
        public void loadpnk(string active, string role, DevExpress.XtraGrid.GridControl gridControl1, GridView gridView1, TextEdit txtsct, ComboBoxEdit cbldt, LookUpEdit ledv, DateEdit denct, DateEdit denht,
            ...
            {
                DataTable da = new DataTable();
                da = gen.GetTable("select  InventoryItemCode,InventoryItemName,Quantity,QuantityConvert,UnitPriceOC,UnitPriceConvert,Description,QuantityExits,QuantityConvertExits from INInwardDetail a,InventoryItem b where a.InventoryItemID=b.InventoryItemID and RefID='" + role + "' order by SortOrder");
                for (int i = 0; i < da.Rows.Count; i++)
                {
                    DataRow dr = dt.NewRow();
                    dr[0] = da.Rows[i][0].ToString();
        ...
        public void checkpnk(string active, string role, Frm_phieunhapkho F, GridView gridView1, LookUpEdit ledt, LookUpEdit ledv, ComboBoxEdit cbldt, TextEdit txtsct, TextEdit txtname, TextEdit txtdc,
           ...
           ToolStripButton tsbtcat, ToolStripSplitButton tsbtin, ToolStripButton tsbtsua, ToolStripButton tsbtnap, string ngaychungtu, TextEdit txtmst, TextEdit txtptvc, string userid, string branchid, LookUpEdit lenv, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (active == "0")
                    {
                        ...
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into INInwardDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,QuantityExits,QuantityConvertExits,UnitPriceOC,UnitPriceConvert) values(newid(),'" + refid + "','" + detail[i, 1] + "','" + detail[i, 2] + "'," + i + ",'" + detail[i, 0] + "',N'" + detail[i, 3] + "',0,0,'" + detail[i, 6] + "','" + detail[i, 7] + "')");
                        }
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into INInwardDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,QuantityExits,QuantityConvertExits,UnitPriceOC,UnitPriceConvert) values(newid(),'" + role + "','" + detail[i, 1] + "','" + detail[i, 2] + "'," + i + ",'" + detail[i, 0] + "',N'" + detail[i, 3] + "','" + detail[i, 4] + "','" + detail[i, 5] + "','" + detail[i, 6] + "','" + detail[i, 7] + "')");
                        }

                        Double ton = 0;
                        try
                        {
                            ton = Double.Parse(gen.GetString("select sum(QuantityConvert-QuantityConvertExits) from INInwardDetail where RefID='" + role + "'"));
                        }
                        catch { }
                        if (ton == 0)
                            gen.ExcuteNonquery("update INInward set IsExport='True' where RefID='" + role + "'");
                        else
                            gen.ExcuteNonquery("update INInward set IsExport='False' where RefID='" + role + "'");
                    }
                    //F.myac();
                    F.getactive("1");
                    F.Text = "Xem phiếu nhập kho";
        ...
        public void tsbtdeletepnk(DevExpress.XtraGrid.Views.Grid.GridView view, Form1 F)
        {
            try
            {
                ...
                if (hoadon == "False")
                {
                    try
                    {
                        //Double temp = Double.Parse(gen.GetString("select sum(QuantityConvertExits) as QuantityConvertExits  from  INInwardDetail where RefID= '" + name + "'"));
                        Double temp = 0;
                        if (temp != 0)
                        {
        ...
        public void checktruoc(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_phieunhapkho F, string ngay, string mk)
        {
            try
            {
                ...
                string id;
                if (vt == 0)
                    id = gen.GetString("select Top 1 * from INInward where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and StockID='" + idkho + "' order by RefNo DESC");
                else
                {
                    id = gen.GetString("select Top 1 * from INInward where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and StockID='" + idkho + "' order by RefNo ASC");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INInward where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and StockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
