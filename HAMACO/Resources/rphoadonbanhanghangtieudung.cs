...
{
    public partial class rphoadonbanhanghangtieudung : DevExpress.XtraReports.UI.XtraReport
    {
        public rphoadonbanhanghangtieudung()
        {
            InitializeComponent();
        }
        gencon gen = new gencon();
        string ma = null;
        public void gettieude(string ngayhoadon, string mst, string nguoinop, string donvi, string diachi, string thanhtoan, Double tongtienhang, Double tienthue, Double thue, Double tongtien, string hoten, string sotienchu, string co, string kho, string phieu, string makhach, string ghichu, string inten)
        {
            ...
            if (gen.GetString("select Top 1 CompanyTaxCode from Center") == "18001113092")
            {
                if (inten != "True")
                    xrLabel9.Text = hoten;
            }
            ...
            try
            {
                DataTable temp = gen.GetTable("select InvName,Description,Code,Province from Stock where StockID='" + kho + "'");
                {
                    if (temp.Rows[0][3].ToString() != "CT")
                    {
                        ...
                        if (gen.GetString("select Top 1 CompanyTaxCode from Center") == "18001113092" && ghichu == "hdbhbangke")
                        {
                            PageHeader.HeightF = 210;
                        }
                    }
                }
            }
