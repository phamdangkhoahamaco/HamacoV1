    ...
    class phieuchuyenkhonblpg
    {
        ...
        public void loadpck(DevExpress.XtraGrid.GridControl lvpq, DevExpress.XtraGrid.Views.Grid.GridView view, string sql)
        {
            ...
            for (int i = 0; i < temp.Rows.Count; i++)
            {
                ...
                refid = gen.GetTable("select * from INTransferSU where RefID='" + temp.Rows[i][42].ToString() + "'");
        ...
        public void tsbtpck(string a, Form1 F, DevExpress.XtraGrid.Views.Grid.GridView view,string roleid,string subsys, string ngaychungtu, string userid, string branchid,string tsbt)
        {
            ...
            u.myac = new Frm_chuyenkhonblpg.ac(F.refreshcknblpg);
            u.getactive(a);
            u.getsub(subsys);
            u.getroleid(roleid);
            if (tsbt == "tsbtpncknblpg")
                u.getpt(tsbt);
            else
                u.getpt("pck");
            ...
            catch { MessageBox.Show("Vui lòng chọn phiếu xuất chuyển nội bộ LPG trước khi sửa."); }
        }
            ...
            TextEdit txtnhd, TextEdit txtms, TextEdit txtkhhd, TextEdit txtshd, DevExpress.XtraGrid.GridControl gridControl2, GridView gridView2, TextEdit txtpxv, TextEdit txtpnv, TextEdit txtcthv, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau,string pt)
        {
            ...
            {
                DataTable da = new DataTable();
                da = gen.GetTable("select  InventoryItemCode,Quantity,QuantityConvert,Description,InventoryItemName,a.UnitPrice,a.Amount,Cost from INTransferDetail a,InventoryItem b where a.InventoryItemID=b.InventoryItemID and RefID='" + role + "' order by SortOrder");
                for (int i = 0; i < da.Rows.Count; i++)
                {
                    DataRow dr = dt.NewRow();
                    dr[0] = da.Rows[i][0].ToString();
                ...
                da = gen.GetTable("select  InventoryItemCode,Quantity,Description,InventoryItemName,a.UnitPrice,a.Amount from INTransferSUDetail a,InventoryItem b where a.InventoryItemID=b.InventoryItemID and RefID in (select refSUID from INTransfer where refID='" + role + "') order by SortOrder");
                for (int i = 0; i < da.Rows.Count; i++)
                {
                ...
                F.Text = "Xem phiếu xuất chuyển kho nội bộ LPG";

                da = gen.GetTable("select AccountingObjectCode,a.Contactname,JournalMemo,InvSeries,RefDate,PostedDate,RefNo,OutwardStockID,Posted,InvNo,Cancel,ShippingNo,RefNoIn,InwardStockID,No,InvDate,RefSUID  from INTransfer a, AccountingObject b where a.AccountingObjectID=b.AccountingObjectID  and RefID='" + role + "'");
                ...
                da = gen.GetTable("select RefNo,RefNoIn  from INTransferSU where RefID='"+da.Rows[0][16]+"'");
            ...
            {
                F.Text = "Thêm phiếu xuất chuyển kho nội bộ LPG";
                if (role == null)
                    ledv.ItemIndex = 0;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (active == "0")
                    {
                        ...
                        gen.ExcuteNonquery("insert into INTransfer(RefID,RefType,RefDate,PostedDate,RefNo,RefNoIn,AccountingObjectID,AccountingObjectName,Contactname,JournalMemo,Posted,OutwardStockID,InwardStockID,ShippingNo,TotalAmount,CostAmount,No,InvSeries,InvNo,InvDate) values(newid(),101,'" + denct.EditValue.ToString() + "','" + denht.EditValue.ToString() + "','" + txtsct.Text + "','" + txtsctn.Text + "','" + dt + "',N'" + txtname.Text + "',N'" + txtngh.Text + "',N'" + txtldn.Text + "','False','" + dv + "','" + dvn + "','" + txtptvc.Text + "','" + tongtien + "','" + chiphi + "','" + txtms.Text + "','" + txtkhhd.Text + "','" + txtshd.Text + "','" + txtnhd.EditValue.ToString() + "')");
                        string refid = gen.GetString("select * from INTransfer where RefNo='" + txtsct.Text + "'");
                        F.getrole(refid);
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into INTransferDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,UnitPrice,Amount,Cost) values(newid(),'" + refid + "','" + detail[i, 1] + "','" + detail[i, 2] + "'," + i + ",'" + detail[i, 0] + "',N'" + detail[i, 3] + "','" + detail[i, 4] + "','" + detail[i, 5] + "','" + detail[i, 6] + "')");
                        }

                        gen.ExcuteNonquery("insert into INTransferSU(RefID,RefType,RefDate,PostedDate,RefNo,RefNoIn,AccountingObjectID,AccountingObjectName,Contactname,JournalMemo,Posted,OutwardStockID,InwardStockID,ShippingNo,TotalAmount,No,InvSeries,InvNo,InvDate) values(newid(),101,'" + denct.EditValue.ToString() + "','" + denht.EditValue.ToString() + "','" + txtpxv.Text + "','" + txtpnv.Text + "','" + dt + "',N'" + txtname.Text + "',N'" + txtngh.Text + "',N'" + txtldn.Text + "','False','" + dv + "','" + dvn + "','" + txtptvc.Text + "','" + tongtienvo + "','" + txtms.Text + "','" + txtkhhd.Text + "','" + txtshd.Text + "','" + txtnhd.EditValue.ToString() + "')");
                        string refidSU = gen.GetString("select * from INTransferSU where RefNo='" + txtpxv.Text + "'");
                        ...
                        gen.ExcuteNonquery("update INTransfer set RefSUID='" + refidSU + "' where RefID='" + refid + "' ");
                        for (int i = 0; i < gridView2.RowCount; i++)
                        {
                            gen.ExcuteNonquery("insert into INTransferSUDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,UnitPrice,Amount) values(newid(),'" + refidSU + "','" + detail1[i, 1] + "','" + detail1[i, 2] + "'," + i + ",'" + detail1[i, 0] + "',N'" + detail1[i, 3] + "','" + detail1[i, 4] + "','" + detail1[i, 5] + "')");
                        }
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < gridView1.RowCount - 1; i++)
                        {
                            gen.ExcuteNonquery("insert into INTransferDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,UnitPrice,Amount,Cost) values(newid(),'" + role + "','" + detail[i, 1] + "','" + detail[i, 2] + "'," + i + ",'" + detail[i, 0] + "',N'" + detail[i, 3] + "','" + detail[i, 4] + "','" + detail[i, 5] + "','" + detail[i, 6] + "')");
                        }

                        string id = gen.GetString("select refSUID from INTransfer where refID='" + role + "'");
                        ...
                        gen.ExcuteNonquery("delete  from  INTransferSUDetail where RefID='" + id + "'");
                        ...
                        for (int i = 0; i < gridView2.RowCount; i++)
                        {
                            gen.ExcuteNonquery("insert into INTransferSUDetail(RefDetailID,RefID,Quantity,QuantityConvert,SortOrder,InventoryItemID,Description,UnitPrice,Amount) values(newid(),'" + id + "','" + detail1[i, 1] + "','" + detail1[i, 2] + "'," + i + ",'" + detail1[i, 0] + "',N'" + detail1[i, 3] + "','" + detail1[i, 4] + "','" + detail1[i, 5] + "')");
                        }

                    }
                    F.myac();
                    F.getactive("1");
                    F.Text = "Xem phiếu nhập kho nội bộ LPG";
                }
        ...
        public void themsct(string ngaychungtu, TextEdit txtsct, string ledv, string ledvn, TextEdit txtsctn, TextEdit txtpxv, TextEdit txtpnv, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau)
        {
            ...
            string sophieunhan = dvn + "-" + ledvn + "-NKNB";
            string sophieuvo = dv + "-" + ledv + "-XKVT";
            string sophieuvonhan = dvn + "-" + ledvn + "-NKVT";
        ...
        public void tsbtdeletepnk(DevExpress.XtraGrid.Views.Grid.GridView view, Form1 F)
        {
            try
            {
                ...
                if (XtraMessageBox.Show("Bạn có chắc muốn xóa phiếu chuyển kho nội bộ LPG " + view.GetRowCellValue(view.FocusedRowHandle, "Số chứng từ").ToString() + "?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
            ...
            catch { XtraMessageBox.Show("Vui lòng chọn phiếu chuyển kho nội bộ LPG trước khi xóa.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information); }
        }


        public void checktruoc(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_chuyenkhonblpg F, string ngay, string mk,string pt)
        {
            try
            {
                ...
                if (vt == 0)
                    if(pt=="tsbtpncknblpg")
                        id = gen.GetString("select Top 1 * from INTransfer where RefNoIn < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and InwardStockID='" + idkho + "' and RefSUID is not Null order by RefNoIn DESC");        
                    else
                        id = gen.GetString("select Top 1 * from INTransfer where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and OutwardStockID='" + idkho + "' and RefSUID is not Null order by RefNo DESC");

                else
                {
                    if (pt == "tsbtpncknblpg")
                        id = gen.GetString("select Top 1 * from INTransfer where RefNoIn < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and InwardStockID='" + idkho + "' and RefSUID is not Null order by RefNoIn ASC");
                    else
                        id = gen.GetString("select Top 1 * from INTransfer where RefNo < '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngay).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngay).Year.ToString() + "' and OutwardStockID='" + idkho + "' and RefSUID is not Null order by RefNo ASC");
        ...
        public void checksau(string sct, int vt, ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, Frm_chuyenkhonblpg F, string ngay, string mk,string pt)
        {
            try
            {
                string idkho = gen.GetString("select * from Stock where StockCode='" + mk + "'");
        ...
        public void checktruocsau(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INTransfer where RefNo > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and RefSUID is not Null and OutwardStockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
        ...
        public void checktruocsaunhap(ToolStripSplitButton tsbttruoc, ToolStripSplitButton tsbtsau, string mk, string sct, string ngaychungtu)
        {
            ...
            {
                tsbtsau.Enabled = true;
                string id = gen.GetString("select Top 1 * from INTransfer where RefNoIn > '" + sct + "' and Month(RefDate)='" + DateTime.Parse(ngaychungtu).Month.ToString() + "' and Year(RefDate)='" + DateTime.Parse(ngaychungtu).Year.ToString() + "' and RefSUID is not Null and InwardStockID='" + idkho + "'");
            }
            catch
            {
                tsbtsau.Enabled = false;
